  ...
  return new Promise((resolve, reject) => {
    ...
    drive.activity.query({ requestBody: params }, (err, res) => {
      ...
      if (res.data.nextPageToken) {
        getAllData(auth, allFiles, ancestor_name, res.data.nextPageToken).then(
          (resAllFiles) => {
            resolve(resAllFiles);
          }
        );
...
function listDriveActivity(auth) {
  ...
  var result = getAllData(
    auth,
    allFiles,
    "items/0Bya_X1cZJP9Tflp1Tno1M3pJSDVNaENMWDFpUXI0NHpmOU4xZG5lOXFxVGd2dG9wU0FWV0E",
    ""
  );
  result.then(function (data) {
    console.log("Happy : )");
    //fs.writeFile("student_details.txt");
  });
  //   const service = google.driveactivity({ version: "v2", auth });
  //   const params = {
  //     pageSize: 1000,
  //     ancestorName:
  //       "items/0Bya_X1cZJP9TfkhudDRNMWw0WkFqY0E5LUpheWJGVzRJZ2NkdXF2NHl2TXVwQ1JOeFIzXzg",
  //   };
...
// fs.appendFile("details_of_students_in_a_section.txt", `\n`, (err) => {
//   if (err) throw err;
// });
